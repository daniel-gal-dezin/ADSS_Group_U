...
public class BusinessService {
    ...
    //delivery functions:
    public String addDelivery(int brId, int year, int month, int day, String sType, int driverid, int storekeeperid,char lisence, int productId, int itemId){
        if(year == 0 || month == 0 || day == 0)
            return new Response("Invalid date").toJson();
        LocalDate date = LocalDate.of(year,month,day);
        Response r = new Response();
        try{
            r.ReturnValue = String.valueOf(bm.addDelivery(brId,date,sType,driverid,storekeeperid,lisence,productId, itemId));
        }catch (Exception e){
            return new Response(e.getMessage()).toJson();
        }
        return r.toJson();
